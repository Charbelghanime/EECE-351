    ...
    if request.method=='POST':
        ...
        if datetime.datetime(int(checkin[:4]), int(checkin[5:7]), int(checkin[8:]), 23, 59, 59) < datetime.datetime.today():
            ...
            if not checkAvailibility(roomType):
                ...
                flash('You have successfully booked the room\n please check your room details', category = 'success')
...
def About():
    ...
    if request.method == 'POST':
        ...
        date = request.form.get('updateDate')
        print(dell)
        if dell:
            ...
            changeRoomNight(current_user.id, request.form.get('checkin'), request.form.get('checkout'))
...
def changeNumberOfPeople(id, num):
    ...
    if room:
        ...
        if room.numOfPeople > 6:
            flash('You cannot be more than 6 guests, please book again!', category = 'error')
            room.numOfPeople = 6
            room.totalPrice = int(room.numOfPeople)*int(room.breakFast)+ int(room.roomType)*20
            return redirect(url_for('auth.About'))
        room.totalPrice = int(room.numOfPeople)*int(room.breakFast)+ int(room.roomType)*20
        db.session.commit()

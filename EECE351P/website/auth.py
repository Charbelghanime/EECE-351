...
def Book():
    ...
    - datetime.datetime(int(checkin[:4]), int(checkin[5:7]), int(checkin[8:]))
        ...
        if datetime.datetime(int(checkin[:4]), int(checkin[5:7]), int(checkin[8:])) <= datetime.datetime.today():
        ...
        elif int(numofpeople) > 2 and int(roomType) == 1:
            ...
            if not checkAvailibility(roomType):
                ...
                flash('You have successfully booked the room please check your registration details', category = 'success')
...
def About():
    ...
    if request.method == 'POST':
        ...
        if people or checkin:
            if people:
                changeNumberOfPeople(roomId, request.form.get('updatePeople'))
            if checkin:
                changeRoomNight(roomId, checkin, checkout)
            return redirect(url_for('auth.About'))
        cancelRegistration(roomId)
        if not HotelRoom.query.filter_by(user_id =current_user.id).first():
            return redirect(url_for('auth.Book'))
        return redirect(url_for('auth.About'))
...
@login_required
def cancelRegistration(roomId):
    room = HotelRoom.query.filter_by(id = roomId).first()
    if room:
...
def changeNumberOfPeople(id, num):
    room = HotelRoom.query.filter_by(user_id=current_user.id, id = id).first()
    if room:
